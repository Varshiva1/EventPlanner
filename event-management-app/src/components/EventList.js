...
const EventList = () => {
  ...
  useEffect(() => {
    const fetchEvents = async () => {
      try {
        const response = await axios.get('http://3.0.97.68/api/events');
  ...
  const handleSaveDescription = async () => {
    try {
      await axios.put(`http://3.0.97.68/api/events/${selectedEventId}`, {
        description,
      });
  ...
  const handleDeleteEvent = async (eventId) => {
    try {
      await axios.delete(`http://3.0.97.68/api/events/${eventId}`);
